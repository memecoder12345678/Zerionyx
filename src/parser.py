...
class Parser:
    ...
    def for_expr(self):
        ...
        if self.current_tok.type == TT_NEWLINE:
            print(
                f"{Fore.LIGHTRED_EX}{Style.BRIGHT}DEBUG{Fore.RESET}{Style.RESET_ALL}: Parser.for_expr: parsing multiline for body"
            )
            res.register_advancement()
            self.advance()
            body = res.register(self.statements())
    ...
    def func_def(self):
        ...
        while self.current_tok.type != TT_RPAREN:
            if self.current_tok.type == TT_MUL:
                ...
                if parsing_stage == "vargs" or parsing_stage == "kargs":
                    return res.failure(
                        InvalidSyntaxError(
                            self.current_tok.pos_start,
                            self.current_tok.pos_end,
