...
class ImageViewer(QWidget):  
    ...
    def load_image(self, filepath):
        if filepath.lower().endswith(".gif"):
            ...
            if self.movie.isValid():
                ...
                self.movie.start()
                self.original_size = (
                    self.movie.currentPixmap().size()
                )
                return
            else:
                QMessageBox.warning(
                    self, "Error", "Could not load GIF."
                )
                self.close()
                return
        ...
        if (
            self.original_size.width() > 2000 or self.original_size.height() > 2000
        ):
    ...
    def update_image(self):  
        if self.movie:
            ...
            if (
                ...
                width_ratio = (
                    available_size.width() / self.original_size.width()
                ) 
                height_ratio = (
                    available_size.height() / self.original_size.height()
                )
                self.scale_factor = min(
            ...
            scaled_size = QSize(
                int(current_size.width()), int(current_size.height())
            )  
        ...
        if not self.original_pixmap.isNull(): 
            ...
            new_size = (
                self.original_size * final_scale
            )  
    ...
    def wheelEvent(self, event: QWheelEvent):  
        if (
            self.movie or not self.original_pixmap.isNull()
        ):  
